...
class FollowTarget(ABC, BaseTask):
    ...
    def set_up_scene(self, scene: Scene) -> None:
        ...
        if self._target_orientation is None:
            self._target_orientation = euler_angles_to_quat(np.array([0, 0, 0]))
        if self._target_prim_path is None:
            self._target_prim_path = find_unique_string_name(
                initial_name="/TargetCube", is_unique_fn=lambda x: not is_prim_path_valid(x)
            )
    ...
    def get_observations(self) -> dict:
        ...
        # 1. tool0 링크의 Prim 경로 설정
        # ee_prim_path = "/World/fairino5_v6_robot/wrist3_link/flange/tool0"  # 정확한 경로 확인!
        # ee_prim = get_prim_at_path(ee_prim_path)
        # ee_position, ee_orientation = ee_prim.
        # # ee 의 data를 갖공 와야됨
        ...
        return {
            self._robot.name: {
                ...
                # "ee_quat": np.array(ee_orientation)
            },
