...
from controller.rmpflow_controller import RMPFlowController
from tasks.follow_target_with_rmpflow import FollowTargetWithRmpFlow
...
my_task = FollowTargetWithRmpFlow(name="follow_target_task",
                                  attach_gripper=True,
                                  fr5_usd_path=FR5_USD_PATH,)
my_world.add_task(my_task)
my_world.reset()

task_params = my_world.get_task("follow_target_task").get_params()
fr5_name = task_params["robot_name"]["value"]
target_name = task_params["target_name"]["value"]

my_fr5 = my_world.scene.get_object(fr5_name)
target_cube = my_world._scene.get_object(target_name)
...
target_cube.set_world_pose(position=np.array([0.4, 0.2, 0.5]),
                           orientation=np.array([0.7071068, 0, -0.7071068, 0]))
...
while simulation_app.is_running():
    ...
    if my_world.is_playing():
        ...
        # Gripper Action
        if count % 1000 > 500:
            actions = my_fr5._gripper.open()
        else:
            actions = my_fr5._gripper.close()

        # articulation_controller.apply_action(actions)

        # F/T 센서 데이터 (필요 시)
        F_T_data = (
            my_controller.articulation_rmp._robot_articulation._articulation_view.get_measured_joint_forces()
        )[0, -1, :]
