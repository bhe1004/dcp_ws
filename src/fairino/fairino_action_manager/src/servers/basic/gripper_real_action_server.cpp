...
GripperRealActionServer::GripperRealActionServer(rclcpp::Node::SharedPtr node, const std::string& name, std::shared_ptr<RobotController::FairinoWrapper> mu)
    : ActionServerBase(node, name, mu)
{
    ...
    this->action_server_ = rclcpp_action::create_server<GripperAction>(
        ...
        name, // Action name
        std::bind(&GripperRealActionServer::handle_goal, this, _1, _2),
        std::bind(&GripperRealActionServer::handle_cancel, this, _1),
        std::bind(&GripperRealActionServer::handle_accepted, this, _1)
    );
...
rclcpp_action::GoalResponse GripperRealActionServer::handle_goal(
...
rclcpp_action::CancelResponse GripperRealActionServer::handle_cancel(
...
void GripperRealActionServer::handle_accepted(
    const std::shared_ptr<GoalHandleGripper> goal_handle)
{
...
bool GripperRealActionServer::compute(const rclcpp::Time& /*current_time*/)
{
    if (!control_running_ || !current_goal_handle_ || !current_goal_handle_->is_active()) {
        return false;
    }
    ...
    if (goal->grasp) { // Close Command (95 -> 0)
        percent = static_cast<float>(
            std::min(100.0, (open_pos - mu_->state().gripper.current_real_pos) / open_pos * 100.0)
        );
    }
    else { // Open Command (0 -> 95)
        percent = static_cast<float>(
            std::min(100.0, ((mu_->state().gripper.current_real_pos - close_pos) / open_pos) * 100.0)
        );
    }
